...
def get_time_step(predict_time):
    time_steps = {'15min': 3, '30min': 6, '45min': 9, '1h': 12}
    if predict_time in time_steps:
        return time_steps[predict_time]
    else:
        raise ValueError("Invalid predict time. Choose from '15min', '30min', '45min', '1h'.")
...
if __name__ == "__main__":
    ...
    if (morning_start <= hms <= morning_end) or (evening_start <= hms <= evening_end):
        ...
        flows = [int(((item * f_std) + f_mean) * 0.9) for item in pre]
    else:
        pre = main(pretime)
        speeds = [round(((item * s_std) + s_mean) * 1.01, 2) for item in pre]
        flows = [int(((item * f_std) + f_mean) * 1.01) for item in pre]
    end_time = time.time()

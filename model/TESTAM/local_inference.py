...
def main(time):
    ...
    parser.add_argument('--load_path', type=str,
                        default='model_pth/BEST_PEMS-BAY_1.55.pth')
    ...
    all_batches = list(enumerate(dataloader['test_loader'].get_iterator()))
...
def anomaly_detection(result):
    df = pd.read_csv('data/sensor_locations.csv')
    df_new = pd.DataFrame(
        {'sensorid': df['sensorid'], 'lat': df['lat'], 'lon': df['lon'], 'speed': result})
